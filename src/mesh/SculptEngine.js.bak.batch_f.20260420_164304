...
function _makeBrushFrame(hitNormal) {
  const up = Math.abs(hitNormal.y) > 0.9
    ? new THREE.Vector3(1, 0, 0)
    : new THREE.Vector3(0, 1, 0);
  const tangent = new THREE.Vector3().crossVectors(up, hitNormal).normalize();
  const bitangent = new THREE.Vector3().crossVectors(hitNormal, tangent).normalize();
  return { tangent, bitangent };
}

function sampleBrushAlpha(vp, hitPoint, hitNormal, brush) {
  const alphaType = brush.alphaType || 'none';
  if (alphaType === 'none') return 1.0;

  const { tangent, bitangent } = _makeBrushFrame(hitNormal);
  const rel = vp.clone().sub(hitPoint);
  const rot = brush.alphaRotation || 0.0;
  const scale = brush.alphaScale || 6.0;
  const c = Math.cos(rot), s = Math.sin(rot);

  let u = rel.dot(tangent) * scale;
  let v = rel.dot(bitangent) * scale;
  ...
  switch (alphaType) {
    case 'pores':
      val = 0.6 + 0.4 * Math.sin(u * 8.0) * Math.sin(v * 8.0);
      break;
    case 'stripes':
      val = 0.5 + 0.5 * Math.sin(u * 10.0);
      break;
    case 'noise':
      val = 0.5 + 0.25 * Math.sin(u * 11.7 + v * 3.1) + 0.25 * Math.sin(v * 13.3 - u * 2.7);
      break;
    case 'cracks':
      val = Math.abs(Math.sin(u * 14.0) * Math.cos(v * 3.0));
      val = 1.0 - val;
      break;
    case 'soft':
      val = 1.0;
  ...
  const alphaStrength = brush.alphaStrength ?? 1.0;
  return Math.max(0.0, Math.min(1.0, val * alphaStrength + (1.0 - alphaStrength)));
}
...
export function applySculpt(geometry, hitPoint, hitNormal, brush, options = {}) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const t = dist / radius;
    const alpha = sampleBrushAlpha(vp, hitPoint, hitNormal, brush);
    const radiusScale = 0.65 + radius * 0.75;
    const influence = getFalloff(falloff, t) * alpha * strength * direction * radiusScale * 0.12;
