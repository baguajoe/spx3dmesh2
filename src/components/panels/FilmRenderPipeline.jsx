    ...
    <div style={{flex:1,overflowY:'auto',padding:'10px 12px'}}>
      <Section title='RESOLUTION' color='#ffcc00'>
        <div style={{display:'flex',flexDirection:'column',gap:3,marginBottom:4}}>
          {RESOLUTIONS.map((r,i)=><div key={i} onClick={()=>setResolution(i)} style={{padding:'4px 8px',borderRadius:4,cursor:'pointer',fontSize:9,fontWeight:700,border:`1px solid ${resolution===i?'#ffcc00':C.border}`,background:resolution===i?'rgba(255,204,0,0.1)':C.bg,color:resolution===i?'#ffcc00':C.dim,display:'flex',justifyContent:'space-between'}}><span>{r.label}</span><span className="spnl-dim">{r.w}×{r.h}</span></div>)}
        </div>
      </Section>
      ...
      <Section title='FRAME SEQUENCE' color={C.dim} defaultOpen={false}>
        ...
        <div className="spnl-dim">{frameEnd-frameStart} frames @ {fps}fps = {((frameEnd-frameStart)/fps).toFixed(1)}s</div>
      </Section>
    </div>
  </div>);
}