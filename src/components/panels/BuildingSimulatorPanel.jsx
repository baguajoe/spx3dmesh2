  ...
  return (
    <div className="spnl-root">
      ...
      <div className="spnl-section">
        <label className="spnl-label">Archetype</label>
        <select className="spnl-select" value={archetype} onChange={e=>setArchetype(e.target.value)}>
          {ARCHETYPES.map(a=><option key={a}>{a}</option>)}
        </select>
        <label className="spnl-label">Floors: {floors}</label>
        <input className="spnl-input" type="range" min={1} max={100} value={floors} onChange={e=>setFloors(+e.target.value)}/>
        <label className="spnl-label">Width: {width}m</label>
        <input className="spnl-input" type="range" min={4} max={60} value={width} onChange={e=>setWidth(+e.target.value)}/>
        <label className="spnl-label">Depth: {depth}m</label>
        <input className="spnl-input" type="range" min={4} max={60} value={depth} onChange={e=>setDepth(+e.target.value)}/>
        <label className="spnl-label">Facade Material</label>
        <select className="spnl-select" value={material} onChange={e=>setMaterial(e.target.value)}>
          {MATERIALS_B.map(m=><option key={m}>{m}</option>)}
        </select>
        <label className="spnl-label">Roof Type</label>
        <select className="spnl-select" value={roofType} onChange={e=>setRoofType(e.target.value)}>
          {ROOF_TYPES.map(r=><option key={r}>{r}</option>)}
        </select>
        <label className="spnl-label">Accent Color</label>
      ...
      <button className="spnl-btn" onClick={build}>⚡ Build</button>
