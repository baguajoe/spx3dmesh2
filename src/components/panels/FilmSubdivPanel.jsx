...
function Slider({label,value,min,max,step=1,onChange,unit=''}){return(<div style={{marginBottom:5}}><div style={{display:'flex',justifyContent:'space-between',fontSize:9,color:C.dim,marginBottom:1}}><span>{label}</span><span className="spnl-teal">{Math.round(value)}{unit}</span></div><input type='range' min={min} max={max} step={step} value={value} onChange={e=>onChange(parseFloat(e.target.value))} style={{width:'100%',accentColor:C.teal,cursor:'pointer',height:3}}/></div>);}
  ...
  return(<div className="spnl-btn">
    ...
    <div style={{flex:1,overflowY:'auto',padding:'10px 12px'}}>
      <div style={{background:C.bg,borderRadius:4,padding:'6px 10px',marginBottom:8,border:`1px solid ${C.border}`}}>
        <div className="spnl-dim">POLY COUNT</div>
