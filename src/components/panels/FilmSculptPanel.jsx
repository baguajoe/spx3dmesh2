  ...
  return(<div style={{width:250,background:C.panel,borderRadius:6,border:`1px solid ${C.border}`,fontFamily:C.font,color:C.text,fontSize:11,boxShadow:'0 8px 32px rgba(0,0,0,0.7)',display:'flex',flexDirection:'column',maxHeight:680}}>
    ...
    <div className="spnl-panel-scroll">
      ...
      <Section title='SYMMETRY' color='#88aaff' defaultOpen={false}>
        ...
        <div className="spnl-row">{['x','y','z'].map(ax=><div key={ax} onClick={()=>setSymmetryAxis(ax)} style={{flex:1,padding:'4px',textAlign:'center',borderRadius:4,cursor:'pointer',fontSize:10,fontWeight:700,border:`1px solid ${symmetryAxis===ax?'#88aaff':C.border}`,color:symmetryAxis===ax?'#88aaff':C.dim,background:symmetryAxis===ax?'rgba(136,170,255,0.1)':C.bg}}>{ax.toUpperCase()}</div>)}</div>
      </Section>
