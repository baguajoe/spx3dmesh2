...
export function AnimationPanel({ onApplyFunction,
  ...
  return (
    <div className="spnl-root">
      <div className="spnl-tabs spnl-tabs--6">
      ...
      <div className="spnl-body">
        ...
        {tab === "keyframes" && (<>
          <Section title="Insert Keyframe">
            ...
            <div className="spnl-row">
              ...
              <select className="spnl-select" value={keyType} onChange={e => setKeyType(e.target.value)}>
                {["location","rotation","scale","loc+rot","loc+rot+scale","custom"].map(t => (
                  <option key={t} value={t}>{t.charAt(0).toUpperCase()+t.slice(1)}</option>
                ))}
              </select>
            </div>
            <button className="spnl-btn-full spnl-btn-accent" onClick={() => onApplyFunction("add_keyframe")}>
              ◆ Insert Keyframe (I)
            </button>
          ...
          <Section title="MoCap Retarget">
            <div className="spnl-row">
              ...
              <label className="spnl-import-btn spnl-import-btn--flex">
                Load BVH
                <input type="file" accept=".bvh" className="spx-hidden"
        ...
        {tab === "shapekeys" && (<>
          <Section title="Shape Keys">
            ...
            <button className="spnl-btn-full spnl-btn-accent" onClick={() => onApplyFunction("shapekey_new")}>
              + New Shape Key
            </button>
            <div className="spnl-btn-row">
        ...
        {tab === "drivers" && (<>
          <Section title="New Driver">
            <div className="spnl-row">
              ...
              <select className="spnl-select" value={driverTarget} onChange={e => setDriverTarget(e.target.value)}>
            ...
            <div className="spnl-row"><span className="spnl-label">Expression</span></div>
            <input className="spnl-input spnl-input--full" type="text" value={driverExpr}
              onChange={e => setDriverExpr(e.target.value)} placeholder="sin(frame * 0.1)" />
            <div className="spnl-driver-presets">
              {["sin(frame*0.1)","cos(frame*0.05)","frame/100","sin(frame*0.2)*2","var*scale"].map(p => (
                <button key={p} className="spnl-driver-preset" onClick={() => setDriverExpr(p)}>{p}</button>
              ))}
            </div>
          ...
          <Section title="Active Drivers">
            ...
            {drivers.length > 0 && (
              <button className="spnl-btn-full" onClick={() => onApplyFunction("apply_drivers")}>Evaluate All Drivers</button>
            )}
          </Section>
        </>)}
        ...
        {tab === "procedural" && (<>
          <Section title="Walk Cycle Generator">
            <div className="spnl-row">
              ...
              <select className="spnl-select" value={walkStyle} onChange={e => setWalkStyle(e.target.value)}>
                {["normal","sneak","run","limp","march","strut"].map(s => (
                  <option key={s} value={s}>{s.charAt(0).toUpperCase()+s.slice(1)}</option>
                ))}
              </select>
            </div>
            <Slider label="Speed" value={walkSpeed} min={0.1} max={3} step={0.1} onChange={setWalkSpeed} />
            <Slider label="Stride" value={walkStride} min={0.1} max={3} step={0.1} onChange={setWalkStride} />
            <div className="spnl-btn-row">
          ...
          <Section title="Procedural Anim">
            <div className="spnl-row">
              <span className="spnl-label">Type</span>
              <select className="spnl-select" value={procType} onChange={e => setProcType(e.target.value)}>
            ...
            <button className="spnl-btn-full" onClick={() => onApplyFunction("proc_audio")}>🎵 Audio Reactive</button>
          </Section>
        ...
        {tab === "motion" && (
          <div className="spnl-motion-wrap">
            <MotionLibraryPanel
              onOpenGamepadAnimator={() => window.dispatchEvent(new CustomEvent('spx:openGamepadAnimator'))}
              onClipApplied={(id, meta) => onApplyFunction && onApplyFunction("motionClipApplied", { id, meta })}
            />
          </div>
        )}
